...
try:
    ...
    # flash-attn is CUDA-only (needs a GPU + the flash-attn package built against
    # your CUDA toolkit) and is not installed here. Falls back to the vanilla
    # scaled_dot_product_attention path the original file already had commented
    # out below - mathematically the same operation, just without the fused
    # CUDA kernel's speed/memory advantage. This is what lets you smoke-test
    # (and, if you don't mind the slower/heavier attention, actually train) the
    # surface-only branch on a CPU-only machine.
...
class WindowAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            ...
            ### use vanilla attn (CPU / no flash-attn fallback) ###
