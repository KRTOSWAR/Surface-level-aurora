...
""" Fanny Lehmann (June 16, 2025): The only modification to the original code is to return the latent variable in addition to the prediction """
...
from aurora.model.surface_encoder import SurfaceOnlyEncoder
from aurora.model.surface_decoder import SurfaceOnlyMLPDecoder
from aurora.model.lora import LoRAMode
from aurora.model.swin3d import BasicLayer3D, Swin3DTransformerBackbone

# `Perceiver3DEncoder` / `Perceiver3DDecoderLite` (and their `perceiver.py` /
# `transform_data.py` dependencies) are only needed by the standard, full
# multi-level path below - imported lazily inside `__init__`'s `else` branch
# so `surface_only=True` doesn't require any of that machinery to even be
# importable.
...
class AuroraLite(torch.nn.Module):
    ...
    def __init__(
            # args 
        self,
        surf_vars: tuple[str, ...] = ("2t", "10u", "10v", "msl", "steer_u", "steer_v", "shear_u", "shear_v"),
        static_vars: tuple[str, ...] = ("lsm", "z"),
        ...
        autocast: bool = False,
        # Surface-only branch control
        surface_only: bool = True,
        surface_embed_dim: int = 128,
        surface_window_size: tuple[int, int, int] = (1, 6, 12),
        # Which of `surf_vars` the surface-only decoder actually forecasts.
        # The rest (e.g. "2t", "steer_u", "steer_v", "shear_u", "shear_v") are
        # still encoder inputs - conditioning the model on more of the
        # environment without asking it to reconstruct them next-step too.
        surface_predict_vars: tuple[str, ...] = ("msl", "10u", "10v", "2t"),
    ) -> None:
        ...
        if surface_only:
            # Lightweight surface-only encoder + compact Swin backbone
            self.encoder = SurfaceOnlyEncoder(
                surf_vars=surf_vars,
                static_vars=static_vars,
                patch_size=patch_size,
                embed_dim=surface_embed_dim,
                max_history_size=max_history_size,
            )

            # Compact Swin config for surface-only operation
            self.backbone = Swin3DTransformerBackbone(
                window_size=surface_window_size,
                encoder_depths=(1, 2),
            ...
            # SurfaceOnlyMLPDecoder, not Perceiver3DDecoderLite: the latter's
            # level-deaggregation path assumes patch_res[0] > 1 and a non-empty
            # batch.atmos_vars, neither of which hold for this branch (it will
            # raise on `torch.stack([], dim=-1)` for the atmos heads otherwise).
            # embed_dim is `surface_embed_dim * 2` for the same reason as the
            # standard path below: the backbone's final decoder stage
            # concatenates with the first encoder stage's skip connection,
            # doubling the token width.
            self.decoder = SurfaceOnlyMLPDecoder(
                predict_vars=surface_predict_vars,
                patch_size=patch_size,
                embed_dim=surface_embed_dim * 2,
                hidden_dim=surface_embed_dim * 2,
                stats=surf_stats,
            )
        else:
            # full model setup 
            from aurora.model.decoder_lite import Perceiver3DDecoderLite
            ...
            self.decoder = Perceiver3DDecoderLite(
                surf_vars=surf_vars,
                atmos_vars=atmos_vars,
                patch_size=patch_size,
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        pred, latent_decoder = self.decoder(
            x,
            batch,
            lead_time=timedelta(hours=6),
            patch_res=patch_res,
